...
function handler(req, res) {
  ...
  (async () => {
    try {
      ...
      if (u.pathname === '/api/groups/add' && req.method === 'POST') {
        const body = JSON.parse(await readBody(req));
        kindOrThrow(body.kind).load(body.id).addGroup(body.group);
        return sendJson(res, 200, { ok: true });
      }
      if (u.pathname === '/api/groups/remove' && req.method === 'POST') {
        const body = JSON.parse(await readBody(req));
        kindOrThrow(body.kind).load(body.id).removeGroup(body.group);
        return sendJson(res, 200, { ok: true });
      }
      if (u.pathname === '/api/criteria/add' && req.method === 'POST') {
        const body = JSON.parse(await readBody(req));
        const text = (body.text || '').trim();
        if (!text) return sendJson(res, 400, { error: 'text required' });
        Goal.load(body.goalId).addCriterion(text);
        return sendJson(res, 200, { ok: true });
      }
      if (u.pathname === '/api/criteria/edit' && req.method === 'POST') {
        const body = JSON.parse(await readBody(req));
        const text = (body.text || '').trim();
        if (!text) return sendJson(res, 400, { error: 'text required' });
        Goal.load(body.goalId).editCriterion(body.idx, text);
        return sendJson(res, 200, { ok: true });
      }
      ...
      if (u.pathname === '/api/steps/add' && req.method === 'POST') {
        const body = JSON.parse(await readBody(req));
        const text = (body.text || '').trim();
        if (!text) return sendJson(res, 400, { error: 'text required' });
        Task.load(body.taskId).addStep(text);
        return sendJson(res, 200, { ok: true });
      }
      if (u.pathname === '/api/steps/edit' && req.method === 'POST') {
        const body = JSON.parse(await readBody(req));
        const text = (body.text || '').trim();
        if (!text) return sendJson(res, 400, { error: 'text required' });
        Task.load(body.taskId).editStep(body.idx, text);
        return sendJson(res, 200, { ok: true });
      }
      if (u.pathname === '/api/steps/toggle' && req.method === 'POST') {
        const body = JSON.parse(await readBody(req));
        Task.load(body.taskId).toggleStep(body.idx);
        return sendJson(res, 200, { ok: true });
      }
      if (u.pathname === '/api/steps/delete' && req.method === 'POST') {
        const body = JSON.parse(await readBody(req));
        Task.load(body.taskId).deleteStep(body.idx);
        return sendJson(res, 200, { ok: true });
      }
