...
class Goal extends TodoItem {
  ...
  addCriterion(text) {
    const newLine = `  - { text: ${quoteYaml(text)}, done: false }`;
    const { eol, lines } = readLines(this._file);
    let critStart = -1, insertAt = -1;
    for (let i = 0; i < lines.length; i++) {
      if (/^criteria:\s*$/.test(lines[i])) { critStart = i; continue; }
      if (critStart !== -1 && insertAt === -1) {
  ...
  editCriterion(idx, text) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (inCriteria) {
        ...
        if (/^\s+-\s*\{/.test(line)) {
          if (count === idx) {
            lines[i] = line.replace(/text:\s*"(?:[^"\\]|\\.)*"/, `text: ${quoteYaml(text)}`);
            fs.writeFileSync(this._file, lines.join(eol));
            return;
          }
          count++;
